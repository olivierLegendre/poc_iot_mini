...
- **Actuators** (`/actuators`): dynamically generated from device-reference capabilities/mappings (no hardcoded device IDs). Controls and status rows are built from mapped actuators and their latest telemetry. Battery badges follow the same rule as **Battery & Status**: mapped battery values are used for battery-powered devices, and devices without an active battery mapping display `Connected device`.
...
- **Scénarios** (`/scenario`): hardcoded demo automations with persistent on/off toggles (stored in Node-RED file context). Only one scenario can be active at a time. Current demo scenarios target the specific devices `hz_light_zigbee_01`, `sonoff_snzb_03p_01`, and `nous_a1z_01`, using normalized metrics (`luminosité`, `occupation`, `state`) and immediately re-evaluating on toggle changes and fresh metric ingestion. The page also shows live debug inputs (current light, occupancy, actuator state, desired state, and the last decision taken).
